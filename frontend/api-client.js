(function (global) {
  ...
  function createApiClient(options) {
    ...
    return Object.freeze({
      ...
      uploadCandidateImage: (candidateId, file, idempotencyKey, options = {}) => { const form = new FormData(); form.append('file', file, file.name || 'product-image'); return request('POST', `${API_BASE}/candidates/${candidateId}/images`, form, { ownerScoped: true, idempotent: true, idempotencyKey, headers: options.demoSample ? { 'X-Guancha-Demo-Sample': 'true' } : undefined }); },
      ...
      createMerchantReply: (sessionId, reply, idempotencyKey, options = {}) => request('POST', `${API_BASE}/selection-sessions/${sessionId}/merchant-replies`, JSON.stringify(reply), { ownerScoped: true, idempotent: true, idempotencyKey, headers: { 'Content-Type': 'application/json', ...(options.demoSample ? { 'X-Guancha-Demo-Sample': 'true' } : {}) } }),
      getMerchantReply: (replyId) => request('GET', `${API_BASE}/merchant-replies/${replyId}`, null, { ownerScoped: true }),
      rejudgeMerchantReply: (sessionId, idempotencyKey, options = {}) => request('POST', `${API_BASE}/selection-sessions/${sessionId}/rejudge`, '{}', { ownerScoped: true, idempotent: true, idempotencyKey, headers: { 'Content-Type': 'application/json', ...(options.demoSample ? { 'X-Guancha-Demo-Sample': 'true' } : {}) } }),
